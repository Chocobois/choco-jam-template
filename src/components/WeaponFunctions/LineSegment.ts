...
export class LineSegment extends Phaser.GameObjects.Container {
    ...
    constructor(scene: BaseScene,x: number, y: number, endx: number, endy: number){
        super(scene,x,y);
        this.ex = endx;
        this.ey = endy;
        this.scene = scene;
    ...
    hitCheck(p: Player): boolean{
        this.gfx1.clear();
        this.gfx2.clear();
        this.gfx3.clear();

        this.gfx3.fillStyle(0xFFFFFF,0.85);
        let ovec = new Phaser.Math.Vector2(this.x-p.x,this.y-p.y);
        let a = this.inputVec.dot(this.inputVec);
        let b = 2*ovec.dot(this.inputVec);
        let c = ovec.dot(ovec) - (Math.pow(p.radius,2));
        ...
        if(disc < 0) {
            ...
            let n1 = Math.sqrt(Math.pow(p.x-this.x,2)+Math.pow(p.y-this.y,2));
            let n2 = Math.sqrt(Math.pow(p.x-this.ex,2)+Math.pow(p.y-this.ey,2));
            ...
            if((dba1 >= 0) && (dba1 <= 1)){
                ...
                return true;
            } else if ((n1 <= p.radius) && (n2 <= p.radius)){
                nx = 0;
                ny = 0;
                this.gfx3.fillStyle(0xFF0000,0.85);
                this.gfx3.beginPath();
